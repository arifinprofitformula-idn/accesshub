...
>
    <div x-data="{ copied: null }" class="space-y-2.5 sm:space-y-5">
        <section class="ah-panel p-3 sm:p-5 lg:p-6">
            <form method="GET" action="{{ route('app.links.index') }}" class="flex flex-col gap-2 sm:gap-3 lg:flex-row lg:items-end lg:gap-4">
                <div class="flex-1">
                ...
                <div class="grid grid-cols-[1fr_auto_auto_auto] items-center gap-2 sm:contents sm:gap-3">
                    ...
                    <a href="{{ route('app.links.index') }}" class="inline-flex min-h-12 items-center justify-center rounded-2xl border border-white/10 bg-white/10 px-3 text-sm font-semibold text-slate-200 transition hover:bg-white/20 sm:px-5">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 sm:hidden">
